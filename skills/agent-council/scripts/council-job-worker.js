...
function main() {
  ...
  // Persona injection: read persona.txt if present in member directory
  const personaPath = path.join(memberDir, 'persona.txt');
  const personaPrompt = fs.existsSync(personaPath)
  ...
  // Inject persona based on CLI type
  if (personaPrompt) {
    const prog = path.basename(program).toLowerCase();
    if (prog === 'claude') {
