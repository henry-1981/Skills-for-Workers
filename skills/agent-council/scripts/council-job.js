...
function parseCouncilConfig(configPath) {
  ...
  if (council.personas != null) {
    if (typeof council.personas !== 'object' || Array.isArray(council.personas)) {
      exitWithError(`Invalid config in ${configPath}: 'council.personas' must be a mapping`);
    }
    merged.council.personas = council.personas;
  }
...
function cmdStart(options, prompt) {
  ...
  for (const member of members) {
    ...
    if (personaDef && personaDef.system_prompt) {
      fs.writeFileSync(
        path.join(memberDir, 'persona.txt'),
        String(personaDef.system_prompt).trim(),
        'utf8'
      );
    }
